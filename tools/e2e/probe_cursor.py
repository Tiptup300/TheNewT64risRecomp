...
proxy (obj_list_ptr) each step to reveal the item count and wrap behavior."""
import sys
...
def main():
    with Game() as g:
        ...
        _sleep(1.0)
        seq = [("start", g.read("obj_list_ptr"))]
        for i in range(8):
            g.press(Keys.DOWN, hold=0.25)
            _sleep(0.25)
            seq.append((f"down{i+1}", g.read("obj_list_ptr")))
        for i in range(8):
            g.press(Keys.UP, hold=0.25)
            _sleep(0.25)
            seq.append((f"up{i+1}", g.read("obj_list_ptr")))
        print("cursor (obj_list_ptr @0x800D3D00) through DOWN x8 then UP x8:")
        for label, v in seq:
            print(f"   {label:>7}: {v}")
