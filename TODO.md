...
- [ ] FIX map-selector mod — built but non-functional: it pokes 0x8011E4F8, but func_80090E08 actually stores the post-RNG map/level bytes near 0x8011FEE8 / 0x801264F9 (base r1 = 0x8012<<16). Need to (a) RE which of the several func_800A35EC(RNG) results is the 0..7 *background* pick (vs. piece randomizer / mode), (b) force it at the pick site (per-frame from FUN_032F00 is the wrong timing), (c) verify with the user driving gameplay (input injection doesn't work headless).
